...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if recon_guidance and t[0] in model_kwargs['y']['recon_steps']:
            ...
            with torch.enable_grad():
                ...
                cond_grad = torch.autograd.grad(guidance_loss, z, retain_graph=True)[0]  #  * (~inpainting_mask).float()
            ...
            traj_guidance = True
            if traj_guidance:
                model_output = hat_x - w_r * cond_grad / sqrt_alpha_bar
            else:
                model_output = hat_x - (w_r * cond_grad * sqrt_alpha_bar / 2) 
    ...
    def p_sample_loop(
        ...
        """
        assert not (recon_guidance and cond_fn_with_grad), 'Can use either recon guidance or classifier guidance, not both'
        final = None
        ...
        if 'prefix' in model_kwargs['y'].keys():
            final["sample"] = torch.cat([model_kwargs['y']['prefix'], final["sample"]], dim=-1)
