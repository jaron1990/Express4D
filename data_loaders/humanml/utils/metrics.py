...
def compute_jitter(loader, fps=60):
    means = []
    with torch.no_grad():
        for batch in loader:
            #return to [bs, frames, lmk, 3(xyz)]
            batch_reshape = batch['full_output'].reshape((batch['full_output'].shape[0], batch['full_output'].shape[1], -1, 3))
            cal_jitter = (
                (
                    (
                    batch_reshape[:,3:] - 
