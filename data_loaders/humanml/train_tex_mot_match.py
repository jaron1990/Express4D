...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        ...
        meta_root = pjoin(opt.checkpoints_dir, opt.dataset_name, opt.name, 'meta')
        ...
        opt.max_epoch //= 4
    elif opt.dataset_name == 'express4d':
        opt.foot_contact_entries = 0
        opt.dim_pose = 61
        opt.is_continue = False
        opt.min_motion_length = 24
        opt.max_motion_length = 196
        # opt.max_epoch //= 4
    ...
    if opt.dataset_name == 'babel':
        ...
        train_loader = get_dataset_loader('express4d',
                                          batch_size=opt.batch_size, num_frames=480,  # not in use
                                          split='train', hml_mode='evaluator_train',fps=opt.fps, max_motions=opt.max_motions, flip_face_on=opt.flip_face_on)
        val_loader = get_dataset_loader('express4d',
                                        batch_size=opt.batch_size, num_frames=480,  # not in use
                                        split='test', hml_mode='evaluator_train',fps=opt.fps)
    else:
