        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                for t in range(repeat_times):
                    ...
                    # evaluation is preformed with the original vec representation
                    if self.dataset.opt.data_rep == 'mat':
                        sample = sample.cpu().numpy()
                        sample = torch.tensor(self.dataset.t2m_dataset.mat2vec(sample))
