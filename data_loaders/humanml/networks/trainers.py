...
class DecompTrainerV3(object):
    ...
    def forward(self, batch_data):
        if self.opt.dataset_name == 'express4d':
            motions = batch_data[0]
            self.motions = motions.squeeze().permute(0,2,1).detach().to(self.device).float()
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                if it % self.opt.log_every == 0:
                    ...
                    for tag, value in logs.items():
                        logger.logkv(tag, value / self.opt.log_every)
            ...
            for sub_epoch in range(sub_ep, self.opt.max_sub_epoch):
                ...
                for i, batch_data in enumerate(train_loader):
                    ...
                    if it % self.opt.log_every == 0:
                        mean_loss = OrderedDict({'val_loss': val_loss})
                        logger.logkv('val_loss', val_loss)
                        logger.logkv('scheduled_length', schedule_len)
