...
from utils.indices import BlendShapeIDX, BlendShapeIDX_6d_angles

print(5)

# 'shape', 'expression', 'rotation', 'jaw_pose', 'eyes_pose', 'neck_pose', 'translation', 'pose',
...
def stack_pkls(data_path):
    ...
    for ii, f in enumerate(pkl_only):
        ...
        face_state = torch.load(full_path)
        # if ii == 0:
    ...
    all_stacked = face_state

    label = f.split(".")[0]

    filter_length = 7

    #for no filter - can use filter_length=1 with either filter_type
    ...
    # return {label: [shape_stacked, expression_stacked, rotation_stacked, jaw_pose_stacked,
    #                 eyes_pose_stacked, neck_pose_stacked, translation_stacked, all_stacked]}  #, pose_stacked]} #TODO can return as FaceState
