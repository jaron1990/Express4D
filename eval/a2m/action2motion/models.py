...
def load_classifier(input_size_raw, num_classes, device, model_path, hidden_size = 256):
    num_points = 70
    # model = torch.load(model_path, map_location=device)
    classifier =  LSTMClassifier(3 * num_points, hidden_size, 1, num_classes).to(device)
...
def load_classifier_for_fid(input_size_raw, num_classes, device):
    model = torch.load(model_path, map_location=device)
    classifier = LSTMClassifier(3 * num_points, hidden_size, 1, c).to(device)
    classifier.load_state_dict(model["model"])
    classifier.eval()
    return classifier
...
def test():
    ...
    classifier = load_classifier(input_size_raw=54, num_classes=13, device="cuda")
